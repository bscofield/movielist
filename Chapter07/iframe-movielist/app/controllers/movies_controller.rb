    ...
    respond_to do |format|
      format.html do
        if fbsession.ready?
          ...
          shared_interests = @movie.interests.find(:all, 
            :include => :user, 
            :conditions => ['users.facebook_id IN (?)', friend_uids])

          unless shared_interests.empty?
            @friends = fbsession.users_getInfo(
              :uids => shared_interests.map { |int| int.user.facebook_id },
